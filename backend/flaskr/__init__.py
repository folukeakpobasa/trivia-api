...
def create_app(test_config=None):
  ...
  def create_new_question():
        """  Create an endpoint to POST a new question, which will require the question
  ...
  def search_for_questions():
      ...
      try: 
        questions = Question.query.filter(Question\
  ...
  def get_question_under_a_category(question_category): 
    ...
    try:
      # category = Category.query.filter(Category.id == question_category).one_or_none()
        
      questions_under_category = Question.query.filter(Question.category==question_category)\
                .order_by('id').all()
      # current_questions= paginate_questions(request, questions_under_category)
        
      if len(questions_under_category) == 0:
            abort(404)
            
      return jsonify({
        'questions': [question.format() for question in questions_under_category],
  ...
  def next_question():
    ...
    random_questions = Question.query.filter(Question.id == random_question_id).first()
